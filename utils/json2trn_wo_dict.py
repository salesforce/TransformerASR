...
def get_parser():
    ...
    parser.add_argument('--addspkr', help="add speaker information to the output trn file", action='store_true', default=False)
    return parser


def main(args):
    args = get_parser().parse_args(args)
    convert(args.json, args.refs, args.hyps, args.num_spkrs, args.addspkr)


def convert(jsonf, refs, hyps, num_spkrs=1, addspkr=False):
    n_ref = len(refs)
    n_hyp = len(hyps)
    assert n_ref == n_hyp
    ...
    for ns in range(num_spkrs):
        ...
        for x in j['utts']:
            ...
            if addspkr:
                hyp_file.write(seq + " (" + j['utts'][x]['utt2spk'].replace('-', '_') + "-" + x + ")\n")
            else:
                hyp_file.write(seq + " (" + x.replace('-', '_') + ")\n")
            ...
            # Weiran: I am adding speaker info here as well, for swbd scoring to work.
            if addspkr:
