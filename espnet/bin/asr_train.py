...
# Copyright 2020 Weiran Wang (Salesforce Research)
...
def get_parser(parser=None, required=True):
    ...
    # loss related
    parser.add_argument('--ctc-type', default='warpctc', type=str,
...
def main(cmd_args):
    ...
    if args.verbose > 0:
        logging.basicConfig(
            level=logging.DEBUG, format='[rank=' + str(args.local_rank) + '] %(asctime)s (%(module)s:%(lineno)d) %(levelname)s: %(message)s',
            handlers=[logging.FileHandler(args.outdir.rstrip("/") + "/train.log"), logging.StreamHandler(sys.stdout)])
    else:
        logging.basicConfig(
            level=logging.WARN, format='[rank=' + str(args.local_rank) + '] %(asctime)s (%(module)s:%(lineno)d) %(levelname)s: %(message)s',
            handlers=[logging.FileHandler(args.outdir.rstrip("/") + "/train.log"), logging.StreamHandler(sys.stdout)])
        if args.local_rank==0:
            logging.warning('Skip DEBUG messages')
    ...
    from espnet.asr.pytorch_backend.asr_dsl import train
    train(args)
