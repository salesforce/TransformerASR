...
def batchfy_by_seq(
        ...
        ikey="input", iaxis=0, okey="output", oaxis=0, ensure_multiple_of=1):
    ...
    while True:
        ...
        # Weiran: this modification is to make sure the batch size can be divided by number of GPUs.
        bs = bs//ensure_multiple_of * ensure_multiple_of
        assert bs>0, ("Weiran: can not make bs to be multiple of %d" % ensure_multiple_of)
...
def batchfy_by_seq_random(
    ...
    if len(data) < min_batch_size:
        raise ValueError(f"#utts({len(data)}) is less than min_batch_size({min_batch_size}).")

    sorted_data = random.sample(data, len(data))
    # make list of minibatches
    minibatches = []
    start = 0
    while True:
        # _, info = sorted_data[start]
        tmp_end = min(start + batch_size, len(sorted_data))
        ilens = [int(info[1][ikey][iaxis]['shape'][0]) for info in sorted_data[start:tmp_end]]
        olens = [int(info[1][okey][oaxis]['shape'][0]) if oaxis >= 0 else max(map(lambda x: int(x['shape'][0]), info[okey]))
                 for info in sorted_data[start:tmp_end]]
        # pdb.set_trace()
        check = np.logical_and(np.cumsum(np.array(ilens)) < (batch_size * max_length_in),
                        np.cumsum(np.array(olens)) < (batch_size * max_length_out))
        bs = max(min_batch_size, np.sum(check))
        # Weiran: this modification is to make sure the batch size can be divided by number of GPUs.
        bs = bs//ensure_multiple_of * ensure_multiple_of
        assert bs>0, ("Weiran: can not make bs to be multiple of %d" % ensure_multiple_of)
        end = min(len(sorted_data), start + bs)
...
BATCH_COUNT_CHOICES = ["auto", "seq", "bin", "frame", "seq_random"]
...
def make_batchset(data, batch_size=0, max_length_in=float("inf"), max_length_out=float("inf"),
                  ...
                  iaxis=0, oaxis=0, ensure_multiple_of=1, seed=0):
    """Make batch set from json dictionary
    ...
    for d in category2data.values():
        ...
        if count == "seq_random":
            batches = batchfy_by_seq_random(
                ...
                ikey=ikey, iaxis=iaxis, okey=okey, oaxis=oaxis, ensure_multiple_of=ensure_multiple_of)
        if count == "seq":
