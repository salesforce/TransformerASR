...
def label_smoothing_dist(odim, lsm_type, transcript=None, blank=0):
    ...
    if lsm_type == 'unigram':
        ...
        for k, v in trans_json.items():
            ...
            # to avoid an error when there is no text in an utterance
            if len(ids) > 0:
                labelcount[ids] += 1
...
class ErrorCalculator(object):
    ...
    def __call__(self, ys_hat, ys_pad, is_ctc=False):
        ...
        logging.debug("converted prediction for evaluating cer/wer:")
        logging.info(seqs_hat)
        logging.debug("converted groundtruth for evaluating cer/wer:")
        logging.info(seqs_true)
        if self.report_cer:
