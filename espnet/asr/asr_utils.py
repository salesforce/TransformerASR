...
def parse_hypothesis(hyp, char_list):
    ...
    """
    # Weiran: remove sos and get results; but eos is not removed.
...
def add_results_to_json(js, nbest_hyps, char_list, add_hyp_prefix_wer=False, copy_input=False, copy_times=False):
    ...
    for n, hyp in enumerate(nbest_hyps, 1):
        ...
        # This is useful in the streaming mode, for debugging the time points.
        if copy_times:
            out_dic["ctc_times"] = " ".join([str(_) for _ in hyp["ctc_times"]])
            out_dic["att_times"] = " ".join([str(_) for _ in hyp["att_times"]])
