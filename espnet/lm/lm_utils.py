...
# Copyright 2020 Salesforce Research (Weiran Wang). Added lexical tree construction with a lexicon file.
#
...
def make_lexical_tree_with_lexicon(word_dict, subword_dict, word_unk, word_eos, lexicon_dict):
    """Make a lexical tree to compute word-level probabilities
    lexicon is a dict of symbols (rather than indices).
    """
    # node [dict(subword_id -> node), list of word_ids, word_set[start-1, end]]
    with open(lexicon_dict, "r") as fin:
    ...
    for line in lines:
        ...
        if w not in word_dict:
            logging.debug("word *%s* does not appear in word_dict" % w)
            continue

        wid = word_dict[w]
        pron = fields[1:]
        if wid > 0 and wid != word_eos:  # skip <blank> and <unk>
            if True in [c not in subword_dict for c in pron]:  # skip unknown subword
                continue
            succ = root[0]  # get successors from root node
            for i, c in enumerate(pron):
                ...
                if cid not in succ:  # if next node does not exist, make a new node
                    ...
                    prev = succ[cid][2]
                    succ[cid][2] = (min(prev[0], wid - 1), max(prev[1], wid))
                if i == len(pron) - 1:  # if word end, set word id
                    succ[cid][1].append(wid)
                succ = succ[cid][0]  # move to the child successors
            count += 1
    logging.info("Build a lexical tree of %d words" % count)
    return root