...
if __name__ == '__main__':
    ...
    if len(sys.argv) < 4 or len(sys.argv) > 5:
        ...
        print("<unk_word>     - the word which represents OOVs in the input text, will uses its pronunciation in the lexicon")
        print("[space_word]   - optional, the word representing spaces in the output phone transcripts")
        exit(1)
    ...
    for line in fread.readlines():
        ...
        word = splits[0]
        """
        # Weiran: only use the first pronunciation.
        if word in pronun:
            continue
        """
        letters = ''
    ...
    for line in fread.readlines():
        ...
        for n in range(0, len(splits)):
            try:
                out_line += pronun[splits[n]]
            except Exception:
                out_line += pronun[unk_word]
